...
const long Principale::ID_BUTTONAMI = wxNewId();
...
Principale::Principale(wxWindow* parent,wxWindowID id)
{
    ...
    Create(parent, wxID_ANY, "Notre chifoumi", wxDefaultPosition, wxDefaultSize, wxDEFAULT_FRAME_STYLE, _T("wxID_ANY"));

    this->SetBackgroundColour(wxColor( * wxRED));

    //Initialisation du nom J1
    this->nomJ1 = wxString("Nous");
    ...
    ItemQuitter = new wxMenuItem(MenuFichier, IDMENUQUIT, _("Deserter\tAlt-F4"), _("Quit the application"), wxITEM_NORMAL);
    MenuFichier->Append(ItemQuitter);
    TopMenu->Append(MenuFichier, _("Actions"));

    //Mise en place du menu paramètres et de ses items
    MenuParametres = new wxMenu();
    ItemAuSujetDe = new wxMenuItem(MenuParametres, IDMENUPARAM, _("Changement d'identité\tAlt-F1"), _("Paramètrer nom du J1"), wxITEM_NORMAL);
    MenuParametres->Append(ItemAuSujetDe);
    TopMenu->Append(MenuParametres, _("Patrie"));

    //Mise en place du menu aide et de ses items
    MenuAide = new wxMenu();
    ItemAuSujetDe = new wxMenuItem(MenuAide, IDMENUABOUT, _("A propos du parti..\tF12"), _("Show info about this application"), wxITEM_NORMAL);
    MenuAide->Append(ItemAuSujetDe);
    TopMenu->Append(MenuAide, _("Manifeste"));
    ...
    //LabelJ2
    labelJ2 = new wxStaticText(this, ID_LABELJ2, _("Le capitalisme"), wxPoint(320,16), wxDefaultSize, 0, _T("ID_LABELJ2"));
    ...
    signeJ2 = new wxStaticBitmap(this, ID_IDSIGNE2, wxBitmap(wxImage(_T("rienJ2.gif")).Rescale(wxSize(104,112).GetWidth(),wxSize(104,112).GetHeight())), wxPoint(320,88), wxSize(104,112), wxSIMPLE_BORDER, _T("ID_IDSIGNE2"));
    signeJ1 = new wxStaticBitmap(this, ID_SIGNEJ1, wxBitmap(wxImage(_T("rienJ1.gif")).Rescale(wxSize(104,112).GetWidth(),wxSize(104,112).GetHeight())), wxPoint(32,88), wxSize(104,112), wxSIMPLE_BORDER, _T("ID_SIGNEJ1"));
    ...
    boutonAppelAmi = new wxButton(this, ID_BUTTONAMI, _("A l'aide !"), wxPoint(328,384), wxSize(112,24), 0, wxDefaultValidator, _T("ID_BUTTONAMI"));
    ...
    Connect(ID_BUTTONAMI,wxEVT_COMMAND_BUTTON_CLICKED,(wxObjectEventFunction)&Principale::OnClickboutonAmi);
    ...
    //SizerBoutonsSignes
    sizerSignes = new wxStaticBoxSizer (wxHORIZONTAL, this, wxT("Pour la gloire de l'URSS, cliquez sur une des images ci-dessous :"));
    ...
    boutons->Add(boutonAppelAmi,1,wxALL, 5);
    boutons->Add(boutonNouvellePartie,1,wxALL, 5);
    ...
    topSizer->Add(boutons, 0, wxALIGN_CENTER_HORIZONTAL|wxEXPAND|wxALL, 10);


    PlaySound(TEXT("hymne.wav"), NULL, SND_ASYNC);
    ...
    topSizer->SetSizeHints(this); //Oui
    this->SetMaxSize(this->GetBestFittingSize());
}
...
void Principale::OnClickboutonAmi(wxCommandEvent& event)
{
...
void Principale::MajSignes()
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                if (this->jeu->getSigneJ1() == Jeu::chuck)
                {
                    this->signeJ1->SetBitmap(wxBitmap(wxImage(_T("puit.gif"))));
                }
            }
        }
    }
...
{
    this->signeJ1->SetBitmap(wxBitmap(wxImage(_T("rienJ1.gif"))));
    this->signeJ2->SetBitmap(wxBitmap(wxImage(_T("rienJ2.gif"))));
}
...
void Principale::OnAide(wxCommandEvent& event)
{
    ...
    wxMessageBox(wxT("Pour la gloire du communisme et de la patrie.\nK. Marx"), _("A propos du bloc Est.."));
}

void Principale::OnParametre(wxCommandEvent& event)
{
    this->parametres = new fenetreJoueur(this); //On instancie la fenetre enfant
    this->parametres->setFenetrePrincipale(this);
    parametres->Show(TRUE); // On l'affiche
}
...
fenetreJoueur* Principale::getFenetreEnfant()
{
    return this->parametres;
}
void Principale::setFenetreEnfant(fenetreJoueur* fenetre)
{
    this->parametres = fenetre;
}
