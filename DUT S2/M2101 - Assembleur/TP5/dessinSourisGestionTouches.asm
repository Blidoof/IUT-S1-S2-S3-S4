...
boucle:
	...
	boucleTouches:
		...
		BEQ suite
		CMP R0,%10000101 ;Si on relache A
		BEQ Aappuye
		CMP R0,%10000110 ;Si on relache B
		BEQ Bappuye
		JMP boucleTouches
	...
	Aappuye:
		...
		;On réaffiche la palette
		CALL paletteCouleur
		
		;On reinitialise le premier clic
		ST R0,premier_clic
		
		;On attend une nouvelle action
		JMP boucleTouches
...
paletteCouleur:
	...
	LD R1, 0 ;Compteur du nombre de bandes déssinées
	LD R0,0 ;Mise a 0 de x et y et couleur (pour que la boucle apparaisse au bon endroit et que les couleurs soient dans le bon ordre)
	ST R0,x
	ST R0,y
	ST R0,couleur
