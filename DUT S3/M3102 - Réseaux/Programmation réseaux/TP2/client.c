...
//VARIABLES
    ...
    int portDistant = 11500; // Port auquel se connecter
...
//TRAITEMENTS
    ...
    adresseDistante.sin_addr.s_addr = inet_addr(adresseD);  // remplissage adr ip
    ...
    while (strcmp(reponse, "QUIT") > 0 || strcmp(reponse, "QUIT") < 0)
    {
        printf(reponse);

        printf("Case a cocher ?\n");

        scanf(" %s", requete);

        send(sock, (void*)requete, sizeof(requete), sizeof(requete));

        recv(sock, (void*)reponse, sizeof(reponse), 0);

        printf(reponse);

        if ((strcmp(reponse, "QUIT") > 0 || strcmp(reponse, "QUIT") < 0) && (strcmp(reponse, "ERREUR") > 0 || strcmp(reponse, "ERREUR") < 0)) {

            sprintf(requete, "%s", "ACK");
